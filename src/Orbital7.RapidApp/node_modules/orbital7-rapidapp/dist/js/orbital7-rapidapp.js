...
function resizeAll() {
    ...
    $(".ra-fullheight:visible").each(function () {
        ...
        //    if (!additionalOffset)
        //        additionalOffset = 0;
        //    var temp = mywindow.innerHeight() - offset.top - additionalOffset - _raFullHeightBottomOffset + "px";
        //    container.css("height", temp);
        //}
    });
}

function setFullHeight(container) {

    alert(container.css("top"));
    container.each(function () {
        container.css("height", "calc(100% - " + this.offsetTop + "px)");
    });
}

function setupDataGrid(id, fullHeight, updateRowColors) {

    var table = $("#" + id);
    updateDataGrid(table, fullHeight, updateRowColors);
}

function updateDataGrid(table, fullHeight, updateRowColors) {
    ...
    if (table.attr("data-width-set") !== "true") {
        ...
        var tableOffset = table.position().top - tableParent.position().top;
        table.css("height", "calc(100% - " + tableOffset + "px + " +
            tableParent.css("padding-top") + ")");
...
function updateDataGridRowColors(table) {
    ...
    tableBody.find("tr").addClass("ra-datagrid-row");
    tableBody.find("tr").removeClass("ra-datagrid-row-alt");
    tableBody.find("tr:odd").addClass("ra-datagrid-row-alt");
}
...
function showModalDialog(contentUrl, returnAction, dialogTitle,
    ...
    if (contentUrl.indexOf("%7Bdatagrid-selected-row-id%7D") !== -1) {
        var selectedRows = $(source).closest(".ra-parent").find(".ra-datagrid-row-selected");
...
function updateBindings() {
    ...
    $(window).resize(function () {
        ...
        $(".ra-datagrid-table").each(function () {

            //var table = $(this),
            //    headingCells = table.find("thead tr:first").children(),
            ...
            //    var max = colWidthBody[i];
            //    if (colWidthHeading[i] > max)
            //        max = colWidthHeading[i];
            //    $(v).width(max);
            //});
            ...
            //// Set the css height of tbody.
            //table.find("tbody").css("height", "calc(100% - 5px - " +
            //    $(headingCells[0]).outerHeight() + "px)");
            
        });

    }).resize().resize(); // Trigger resize handler TWICE.
    ...
    $(".ra-container-scrollable").overlayScrollbars({
        overflowBehavior: {
            x: "scroll",
            y: "scroll"
        }
    ...
    //        //if (!additionalOffset)
    //        //    additionalOffset = 0;
    //        container.css("height", "calc(100% - 2px - " + offset.top + "px);");
...
(function () {
    ...
    Tablesort.prototype = {
        ...
        sortTable: function (header, update) {
            ...
            updateDataGrid($(that.table), true, true);   // JVE Added.
