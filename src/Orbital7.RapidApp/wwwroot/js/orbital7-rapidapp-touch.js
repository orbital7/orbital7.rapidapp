        ...
        $.get({
            ...
            error: function (xhr) {
                if (xhr.responseText)
                    ...
                    container.html("<strong class='red'>ERROR:</strong> Dialog content could not be loaded");
            }
        });
    }
}
...
function setTouchInputFieldCell(type, value, displayValue, displayElement, cellElement, setPlaceHolder) {
    ...
    if (type === "pin") {
        ...
        if (value !== "") {  // if (value !== "") -> problem for Read-Only content.
            displayElement.addClass("ra-touchinput-field-value-content");
            cellElement.html("<span style='white-space: nowrap;'>" + value.toPhoneNumber() + "</span>");
        }
        else {
            displayElement.addClass("ra-touchinput-field-value-contentempty");
            if (setPlaceHolder)
